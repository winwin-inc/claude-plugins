...
| 性能预期 | ✅ Compliant | Python 包安装 < 5秒，文档生成性能由 Claude Code保证 |

### ⚠️ 原则6例外：增量更新原则

**例外批准**: Session 2025-01-04 (Afternoon) 澄清会话

**例外内容**:
- **采用策略**: 完全覆盖（完全重新生成整个文档，不保留手动修改）
- **理由**:
  1. AI 生成文档的完全自动化特性
  2. 用户通过 Git 管理版本历史和自定义修改
  3. 避免复杂的增量逻辑和状态管理
  4. 保持实现简洁，降低维护成本
- **缓解措施**:
  1. 文档头明确说明更新策略
  2. 用户通过 Git diff 查看变更
  3. 提供清晰的文档说明完全覆盖的工作流程
- **适用范围**: 仅限 Claude Code `/wiki-generate` 命令生成的文档
- **记录位置**: spec.md FR-09，plan.md 本章节

### ✅ 合规总结

所有适用原则均合规或已批准例外。职责清晰分离，Python 包专注于工具化，文档生成由 Claude Code AI 处理。增量更新原则的例外已正式记录和批准。
