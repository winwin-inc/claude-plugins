...
### Session 2025-01-04 (Evening)

- Q: wiki-generate.md 命令文件的更新策略 - 项目未发布，现有命令文件使用旧架构，如何处理？ → A: 完全重写 wiki-generate.md 匹配规范 v2.0，采用中文文件名、完全覆盖策略、技术栈显式检测、配置驱动，移除旧的增量更新逻辑，确保架构一致性

### Session 2025-01-04 (Afternoon)

- Q: 中文文档的文件命名策略 - 使用中文、拼音还是英文文件名？ → A: 使用中文文件名（如 `快速开始.md`、`项目概述.md`），提高中文用户可读性，与现代文件系统和 Web 服务器兼容
- Q: 条件文档的自动检测规则 - 基于技术栈、代码规模还是其他判断？ → A: 基于技术栈显式检测（检测到特定框架/库就生成对应文档，如 SQLAlchemy→数据模型），精确可预测
- Q: 文档更新策略和版本控制 - 完全覆盖、增量更新还是标记区域？ → A: 完全覆盖（每次重新生成整个文档，用户通过 Git 管理版本历史），适合纯 AI 生成场景
- Q: 文档生成失败时的错误处理 - 全部回滚、部分成功、交互式还是尽力而为？ → B: 部分成功（保留成功生成的文档，跳过失败的，生成错误报告），用户友好且可恢复
- Q: 生成文档的质量验证标准 - 基础验证、AI 评分、手动审查还是无验证？ → A: 基础自动化验证（检查必需元素、格式规范、链接有效性），快速可靠覆盖常见问题

### Session 2025-01-04 (Morning)
...
- Q: 架构边界和职责划分 - Python 包和 Claude Code 命令如何分工？ → B: Python 包只负责安装命令和模板，文档生成由 Claude Code `/wiki-generate` 命令使用 AI 实现
- Q: Python 包 CLI 的职责范围 - `wiki-generator` 命令应该提供哪些功能？ → A: 只提供初始化（--init）和迁移工具（--migrate），不实现文档生成逻辑
- Q: 模板文件存储位置和格式 - 模板应该如何存储和分发？ → B: 模板作为 `.claude/templates/` 目录下的 Markdown 文件，安装时复制到项目，用户可自定义
- Q: 性能目标的适用范围 - 规范中的性能目标如何理解和验证？ → C: 作为 Claude Code 命令的端到端目标，Python 包只负责快速安装（<5秒）
- Q: 配置验证的职责划分 - 配置文件验证应该由哪一层负责？ → A: Python 包提供验证工具（--validate）和 JSON Schema，Claude Code 命令使用前先验证
...
- 三级目录按功能模块分类：
  ...
  - `功能模块名称/子模块.md`（模块文档）
- **文件命名约定**：
  - 中文文档：使用中文文件名（如 `快速开始.md`、`项目概述.md`），提高可读性
  - 英文文档：使用英文文件名（如 `quickstart.md`、`overview.md`）
  - 模块目录：使用中文模块名（如 `数据模型/`、`系统架构设计/`）

**验收标准**：
- [ ] 生成的目录结构与参考项目一致
- [ ] 顶级文档使用数字前缀排序（00-99）
- [ ] 模块目录名称清晰反映功能
- [ ] 中文文档使用中文文件名，与现代文件系统和 Web 服务器兼容
...
- 支持自定义模块分组
- **条件文档生成规则**（基于技术栈显式检测）：
  - 检测到 SQLAlchemy/Django ORM/SQLAlchemy Core → 生成数据模型文档
  - 检测到 FastAPI/Flask/Django REST Framework → 生成 API 文档
  - 检测到 Celery/RQ → 生成任务队列文档
  - 检测到 pytest/unittest → 生成测试策略文档
  - 检测到 Docker/docker-compose → 生成部署指南文档
  - 用户可以通过配置的 `sections.required` 和 `sections.optional` 覆盖自动检测结果

**验收标准**：
- [ ] 可以选择"参考项目"模板
- [ ] 可以自定义模块分组规则
- [ ] 配置文件格式符合 JSON Schema
- [ ] AI 能基于技术栈准确检测并生成相应的条件文档
...
**需求详情**：
- **完全覆盖策略**：每次重新生成整个文档，不保留用户手动修改
- **版本控制**：用户通过 Git 管理文档版本历史和自定义修改
- **更新触发**：用户手动调用 `/wiki-generate` 命令触发更新
- **适用场景**：适用于完全由 AI 生成的文档场景
...
**需求详情**：
- **部分成功策略**：保留成功生成的文档，跳过失败的文档
- **错误报告**：生成详细的错误报告，列出失败的文档和原因
- **用户友好**：不因个别文档失败而丢失所有进度
- **恢复机制**：用户修复问题后可以重新生成失败的文档

**验收标准**：
- [ ] 成功的文档被保存到输出目录
- [ ] 失败的文档不影响其他文档
- [ ] 生成错误报告文件（如 `docs/generation-errors.md`）
- [ ] 错误消息清晰，包含失败原因和建议修复方案

**FR-11**: 文档质量自动化验证

**需求详情**：
- **基础验证检查**：
  - 必需元素检查：`<cite>` 块、目录索引、Section sources
  - 格式规范检查：Markdown 语法、标题层级、链接格式
  - 链接有效性检查：相对路径链接、file:// 协议链接
- **验证结果**：生成验证报告，列出通过和失败的项目
- **不验证内容**：不验证文档内容的准确性和完整性（由 AI 生成保证）
...
**FR-12**: wiki-generate.md 命令文件实现（v2.0）

**需求详情**：
- **命令文件职责**：`.claude/commands/wiki-generate.md` 是文档生成的核心逻辑，由 Claude Code AI 执行
- **完全重写**：移除 v1.0 的增量更新逻辑，采用规范 v2.0 的所有决策
- **配置驱动**：读取 `.claude/wiki-config.json`，根据配置决定生成行为
- **中文文件名**：生成文档使用中文文件名（如 `快速开始.md`、`项目概述.md`）
- **完全覆盖策略**：每次重新生成整个文档，不保留手动修改
- **技术栈显式检测**：基于检测到的框架/库生成条件文档（SQLAlchemy→数据模型，FastAPI→API 文档）
- **部分成功错误处理**：保留成功生成的文档，跳过失败的，生成错误报告
- **目录结构**：
  - 单语言：`docs/{lang}/content/00-序号-文档名.md`
  - 双语：`docs/zh/content/` 和 `docs/en/content/`

**验收标准**：
- [ ] wiki-generate.md 完全重写，移除所有 v1.0 逻辑
- [ ] 支持配置文件读取和验证
- [ ] 实现技术栈显式检测规则
- [ ] 生成分层目录结构（参考项目标准）
- [ ] 每个文档包含 `<cite>` 块、目录索引、Section sources
- [ ] 实现部分成功错误处理机制
- [ ] 实现基础自动化验证
